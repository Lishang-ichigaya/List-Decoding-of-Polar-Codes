...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 2 and sys.argv[1] == "ber":
        result_file_name = "D_UnfairTwoCRC-SCLの結果2.txt"
        for threshold in [64, 72, 80]:
                ...
                for L in [4]:
                    for N in [256]:
                        ...
                        for i in range(kaisu):
                            ...
                            #hat_message00 = np.delete(decoder0.hat_message, np.s_[threshold-r//2:threshold], 0)
                            #hat_message00 = np.delete(hat_message00, np.s_[k:], 0)

                            hat_message11 = np.delete(decoder1.hat_message, np.s_[threshold-r//2:threshold], 0)
                            hat_message11 = np.delete(hat_message11, np.s_[k:], 0)

                            # hat_message1 = Message(k)
                            # hat_message1.message = decoder1.hat_message
                            # end1 = time.time()

                            #error0 = np.bitwise_xor(message.message, hat_message00)
                            error1 = np.bitwise_xor(message.message, hat_message11)
