...
IW_name = "IW_"+str(N)+"_"+str(snr)+".txt"
...
def getSymmetricChannelCapacity(channel):
    ...
    for x in [0, 1]:
        for y in range(channel.N):
            I += channel.W[x][y] * \
                np.log2(channel.W[x][y]/(channel.W[0][y]+channel.W[1][y]))
    I = 1+0.5*I
    return I
...
if __name__ == "__main__":
    ...
    with open(IW_name, mode='a', encoding='utf-8') as f:
        for i in range(N):
            f.write(str(I_W[i])+" ")
    ...
    try:
        notification.NoticeEnd(
            "差分:"+str(sum_I-N*getSymmetricChannelCapacity(Q_AWGN)))
